...
char* caesarCipher(char* s, int k) {
    ...
    char* encrypted = (char*)malloc(STRING_LENGTH + 1);
      ...
      if(s[i] != toupper(s[i]))
      {
        for(int j = 0; j < ALPHABET_LENGTH; j++)
        {
          if(s[i] == alphabet[j])
          {
            ...
            {
              encrypted[i] = alphabet[j + k - ALPHABET_LENGTH];
              break;
            }
          }
          if(j + 1 == ALPHABET_LENGTH)
          {
            encrypted[i] = s[i];
          }
        }
      ...
      else
      {
        for(int j = 0; j < ALPHABET_LENGTH; j++)
        {
          if(s[i] == upperAlphabet[j])
          {
            if(j + k < ALPHABET_LENGTH)
            {
              encrypted[i] = upperAlphabet[j + k];
              break;
            }
            else
            {
              encrypted[i] = upperAlphabet[j + k - ALPHABET_LENGTH];
              break;
            }
          }
          if(j + 1 == ALPHABET_LENGTH)
